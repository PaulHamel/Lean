    ...
    public class QuoteBarConsolidator : PeriodCountConsolidatorBase<QuoteBar, QuoteBar>
    {
        ...
        protected override void AggregateBar(ref QuoteBar workingBar, QuoteBar data)
        {
            ...
            else if (!IsTimeBased)
            {
                ...
                // because in the `if` above we are already providing the `data.Period` as argument
                // See test 'AggregatesNewCountQuoteBarProperlyDaily' or 'GentlyHandlesPeriodAndDataAreSameResolution' which assert period
